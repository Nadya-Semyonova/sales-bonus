...
function calculateSimpleRevenue(purchase, _product) {
   ...
   discountCalculation = 1 - (purchase.discount / 100);
   return sale_price * quantity * discountCalculation
...
function analyzeSalesData(data, options) { 
    ...
    sellerStats.forEach((seller, index) => {
        ...
        const bestProducts = Object.entries(seller.products_sold)
        .map(([sku, quantity]) => ({ sku, quantity }))
        .sort((a, b) => b.quantity - a.quantity)
        .slice(0, 10);// Формируем топ-10 товаров
        seller.top_products = bestProducts;
});
